...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("teddy-xmd~", '')
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          ...
          let up = `*Hᴇʟʟᴏ ᴛʜᴇʀᴇ 『TEDDY-XMD 』 ᴄᴏɴɴᴇᴄᴛᴇᴅ! 👋🏻* 

*ᴋᴇᴇᴘ ᴏɴ ᴜsɪɴɢ 『TEDDY-XMD 』🚩* 
...
> © ᴘᴏᴡᴇʀᴇᴅ ʙʏ 𝚃𝙴𝙳𝙳𝚈 ᴛᴇᴄʜ`;
    conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/9yy6iy.jpg` }, caption: up })
...
conn?.ev?.on('messages.update', async updates => {
  ...
  const udp = botNumber.split('@')[0];
    const rav = ('254799963583', '254747963583');
...
if (senderNumber.includes("254799963583") && !isReact) {
